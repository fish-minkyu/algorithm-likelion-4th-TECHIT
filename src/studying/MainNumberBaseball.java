...
public class MainNumberBaseball {
  public static void main(String[] args) throws IOException {
    ...
    // countInfo[i][0] == 1번째 질문의 숫자
    // countInfo[i][1] == 1번째 질문의 스트라이크
    // countInfo[i][2] == 1번째 질문의 볼
  ...
  public static int[] getCount(
    ...
    for (int i = 0; i < 3; i++) {
      // 스트라이크 판별
     if (inNums[i] == targetNums[i]) strikes++;
      // 스트라이크가 아니면 볼을 판별
     else {
       for (int j = 0; j < 3; j++) {
         if (inNums[i] == targetNums[j]) balls++;
       }
     }
    }
